										...
										uint32_t maxKeyLength, uint8_t isCriticalKey, uint32_t keyRef, uint8_t matchLength, uint32_t *getProvider)
{
	...
	if (isCriticalKey)
	{
		...
		{
			case 'F': // BISS keys
				cs_log("Key not found: %c %08X %s", identifier, provider, keyName);
				break;
...
static void BissNamespace2str(char *buf, uint8_t len, uint32_t namespace)
{
	// Extract the orbital position & frequency or onid from the supplied namespace.
	...
	uint16_t frequency, degrees, transponder;
	char compass, polarisation, noFreq = 0;

	degrees = (namespace >> 16) & 0x7FFF; // Remove not-a-pid bit/flag

	if (degrees & 0x4000) // nofreq flag
	{
		degrees &= ~0x4000;
		noFreq = 1;
	}
	...
	if (noFreq)
	{
		transponder = namespace & 0xFFFF; // Select our custom "transponder identifier"
		snprintf(buf, len, "%.1f%c t:%04X", degrees / 10.0, compass, transponder); // Degree sign "\u00B0" requires c99 standard
	}
	else
	{
		frequency = namespace & 0x7FFF; // Remove polarity bit
		polarisation = namespace & 0x8000 ? 'V' : 'H';
		snprintf(buf, len, "%.1f%c %5d%c", degrees / 10.0, compass, frequency, polarisation); // Degree sign "\u00B0" requires c99 standard
	}
}
					  ...
					  uint8_t *dw, uint16_t srvid, uint16_t ecmpid)
{
	...
	uint16_t ecmLen = 0, pid = 0, pmtpid;
	uint32_t i, ens, pidsCRC32;
	char tbuf1[32], tbuf2[32], keyValue[17];
	...
	// First try using the unique channel hash (if namespace is present in the ecm)
	if (ecmLen >= 13 && ecmLen <= ecmDataLen)
	{
		memcpy(ecmCopy, ecm, ecmLen);
		ens = b2i(4, ecmCopy + ecmLen - 4); // namespace will be the last 4 bytes
		pmtpid = b2i(2, ecmCopy + 5); // pmtpid follows sid in our extended fake ecm

		if (ens & 0x80000000) // Top bit set: a valid namespace, not a pid
		{
			...
			for (i = 0; i < 5; i++) // Find key matching hash made with frequency modified to: f+0, then f-1, f+1, f-2, lastly f+2
			{
				if (i & 1)
				{
					ecmCopy[ecmLen - 1] -= i; // f-1 MHz, f-2 MHz
				}
				else
				{
					ecmCopy[ecmLen - 1] += i; // f+1 MHz, f+2 MHz
				...
				pidsCRC32 = crc32(0x2600, ecmCopy + 3, ecmLen - 3);
				haveKey1 = FindKey('F', pidsCRC32, 0, "00", dw, 8, 1, 0, 0, NULL);
				BissNamespace2str(tbuf1, sizeof(tbuf1), b2i(4, ecmCopy + ecmLen - 4));

				if (haveKey1)
				{
					memcpy(&dw[8], dw, 8);
					cs_log("Key found: F %08X 00 ; %s pmt:%04X sid:%04X", pidsCRC32, tbuf1, pmtpid, srvid);
					return 0;
				}
				...
				if (i == 0)
				{
					...
					strftime(tbuf2, sizeof(tbuf2), "added: %F @ %R", &timeinfo);

					cs_log("Example key: F %08X 00 XXXXXXXXXXXXXXXX ; %s pmt:%04X sid:%04X %s", pidsCRC32, tbuf1, pmtpid, srvid, tbuf2);
				}

				// nofreq flag - do not iterate if this value is not a frequency
				if (ens & 0x40000000)
				{
					break;
				}
	...
	if (ecmpid != 0x1FFF && ecmpid != 0)
	{
		...
		haveKey2 = FindKey('F', (srvid<<16)|ecmpid, 0, "01", &dw[8], 8, 0, 0, 0, NULL); // Do not print "key not found". "01" keys will be removed in the future
		...
		{
			memcpy(dw, &dw[8], 8);
			cs_log("Key found: F %04X%04X 01", srvid, ecmpid);
			return 0;
		}
	}

	// Try to get the pid from oscam's fake ecm ([sid] [pmtpid] [pid1] [pid2] ... [pidx] [namespace])
	if (ecmLen >= 7 && ecmLen <= ecmDataLen) // Use >= for BISS radio channels with just one (audio) pid
	{
		for (i = 5; i + 1 < ecmLen; i += 2)
		{
			pid = b2i(2, ecm + i);

			if ((pid & 0x8000) && (i + 4 == ecmLen)) // This is the namespace, not a stream pid
			{
				break;
			}

			haveKey1 = FindKey('F', (srvid<<16)|pid, 0, "00", dw, 8, 1, 0, 0, NULL);
			haveKey2 = FindKey('F', (srvid<<16)|pid, 0, "01", &dw[8], 8, 0, 0, 0, NULL); // Do not print "key not found". "01" keys will be removed in the future
			...
			{
				memcpy(dw, &dw[8], 8);
				cs_log("Key found: F %04X%04X 01", srvid, pid);
				return 0;
			}
		}
	}
	...
	if (ecmpid == 0x1FFF || ecmpid == 0)
	{
		...
		haveKey2 = FindKey('F', (srvid<<16)|0x1FFF, 0, "01", &dw[8], 8, 0, 0, 0, NULL); // Do not print "key not found". "01" keys will be removed in the future
		...
		{
			memcpy(dw, &dw[8], 8);
			cs_log("Key found: F %04X1FFF 01", srvid);
			return 0;
		}
	}

	// Default BISS key for events with many feeds sharing same key
	if (ecmpid != 0 && FindKey('F', 0xA11FEED5, 0, "00", dw, 8, 1, 0, 0, NULL)) // Search default key only for local ecm
	{
		memcpy(&dw[8], dw, 8);
...
#endif
{
	...
	uint8_t hashModeCw, needsUnmasking, xorMode;
#ifdef WITH_EMU
	uint8_t *dwp;
